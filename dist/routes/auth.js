...
router.post(
  ...
  (0, errorHandler_1.asyncHandler)(async (req, res, next) => {
    ...
    if (!email || !password) {
      return next(
        new errorHandler_1.AppError("Please provide email and password", 400),
      );
    }
...
router.post(
  ...
  (0, errorHandler_1.asyncHandler)(async (req, res, next) => {
    ...
    if (!token) {
      return next(
        new errorHandler_1.AppError("Google ID token is required", 400),
      );
    }
    try {
      ...
      if (user) {
        if (!user.isActive) {
          return next(
            new errorHandler_1.AppError("Account is deactivated", 403),
          );
        }
      ...
      return next(
        new errorHandler_1.AppError("Google Authentication failed", 401),
      );
    }
  }),
);
...
router.post(
  ...
  (0, errorHandler_1.asyncHandler)(async (req, res, next) => {
    ...
    if (!refreshToken) {
      return next(
        new errorHandler_1.AppError("Refresh token is required", 400),
      );
    }
    try {
      ...
      return next(
        new errorHandler_1.AppError("Refresh token expired or invalid", 401),
      );
    }
  }),
);
...
router.post(
  ...
  (0, errorHandler_1.asyncHandler)(async (req, res, next) => {
    ...
    if (!user) {
      return next(
        new errorHandler_1.AppError("Invalid or expired reset token", 400),
      );
    }
    // Set new password
...
router.get(
  "/",
  authMiddleware_1.protect,
  (0, authMiddleware_1.authorize)("admin"),
