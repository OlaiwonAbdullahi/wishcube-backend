...
import {
  giftSuccessTemplate,
  vendorOrderNotificationTemplate,
  giftRedeemedSenderTemplate,
      ...
      if (vendor?.email) {
        sendEmail({
          ...
          html: vendorOrderNotificationTemplate(
            ...
            `${address}, ${city}, ${state}`,
            phone,
            `${process.env.CLIENT_URL}/vendor/orders/${(order as any)._id}`,
          ),
        }).catch(console.error);
      }
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!token) {
      throw new AppError("Redeem token is required for tracking", 400);
    }

    const gift = await Gift.findOne({ redeemToken: token as string });
    if (!gift) {
      throw new AppError("Invalid gift token", 404);
    }
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!token || !code) {
      throw new AppError("Token and delivery code are required", 400);
    }
    ...
    if (order.deliveryCode !== code) {
      throw new AppError("Invalid delivery confirmation code", 400);
    }
